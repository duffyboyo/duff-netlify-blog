...
export default function Page({ posts, tags, pagination, page }: Props) {
  const url = `/portfolio/page/${page}`;
  const title = "All Entries";
  return (
    <Layout>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const posts = listPortfolioContent(page, config.posts_per_page);
  const tags = listTags();
  const pagination = {
    current: page,
...
export const getStaticPaths: GetStaticPaths = async () => {
  const pages = Math.ceil(countPortfolio() / config.posts_per_page);
